...
class DynamicRagModel(DynamicRagPreTrainedModel):
    ...
    def forward(
        ...
        if decoder_input_ids is not None:
            ...
            decoder_input_ids = decoder_input_ids.to("cuda:{}".format(Config.select_gpu))

        if decoder_attention_mask is not None:
            decoder_attention_mask = decoder_attention_mask.repeat_interleave(n_docs, dim=0)
            decoder_attention_mask = decoder_attention_mask.to("cuda:{}".format(Config.select_gpu))
...
class DynamicRagForGeneration(DynamicRagPreTrainedModel):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, n_docs, gradient_checkpointing):
        if 'bart' in pretrained_model_name_or_path:
            ...
            generator = MBartForConditionalGeneration.from_pretrained(pretrained_model_name_or_path)
