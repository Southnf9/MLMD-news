...
Config =Config()
model  =BertModel.from_pretrained('/root/bert-base-multilingual-cased')
model.to("cuda:{}".format(Config.select_gpu))
...
class HSumGraph(nn.Module):
    ...
    def _init_sn_param(self):
        ...
        self.ngram_enc = sentEncoder(self._hps, model)
...
class MTSumGraph(nn.Module):
    ...
    def set_wnfeature(self, graph):
        ...
        wid = graph.nodes[wnode_id].data["id"]  # [n_wnodes]
        max_length = 256
        chunks = [wid[i:i + max_length] for i in range(0, len(wid), max_length)]
        w_embed = []
        for chunk in chunks:
            chunk = chunk.unsqueeze(0)  # 添加批次维度
            attention_mask = torch.ones(chunk.shape, dtype=torch.long).to(chunk.device)# 创建注意力掩码
            with torch.no_grad():  # 禁用梯度计算以节省内存
                outputs = model(input_ids=chunk, attention_mask=attention_mask)
            w_embed.append(outputs.last_hidden_state)
        w_embed = torch.cat(w_embed, dim=1)[0]
        # w_embed = self._embed(wid)  # [n_wnodes, D]
        graph.nodes[wnode_id].data["embed"] = w_embed
        etf = graph.edges[wsedge_id].data["tffrac"]
        graph.edges[wsedge_id].data["tfidfembed"] = self._TFembed(etf)
